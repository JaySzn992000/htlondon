...
const Collections = ({ addToCart }) => {
  ...
  useEffect(() => {
    axios
      ...
      .then((res) => {
        setAllProducts(res.data);        // ✅ ORIGINAL — never changes
        setFilteredProducts(res.data);   // ✅ initial display
      })
            ...
            <div>
              <div className="flex_productlist">
                {filteredProducts.length === 0 ? (
                  <p style={{ padding: "40px", fontSize: "16px" }}>
                    No products found
                  </p>
                ) : (
                  filteredProducts.map((productlist) => (
                    <div key={productlist.id} className="produclist_divContainer">
                      ...
                      <Link
                        to={`/products/${slugify(productlist.name)}/${productlist.id}`}
                      >
                        <img
