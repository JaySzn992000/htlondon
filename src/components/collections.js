...
if (quickViewTimeoutRef.current) clearTimeout(quickViewTimeoutRef.current);
...
Discover the essence of luxury fashion
</p>
...
{["All", "New Arrivals", "Best Sellers", "Premium", "Limited", "Sale"].map(
(cat, i) => (
<button
key={i}
...
<i
className={`fa ${
wishlistStatus[product.id] ? "fa-heart" : "fa-heart-o"
}`}
></i>
...
<div className="load-more-container">
<p className="load-more-end">No products found</p>
</div>
)}
</>
)}
...
<img
src={quickViewProduct.file_path}
...
"Experience the perfect blend of style and comfort with this premium piece. Crafted with attention to every detail, this product embodies sophistication and modern elegance."}
</p>
...
<button
className={`qv-wishlist ${
wishlistStatus[quickViewProduct.id] ? "active" : ""
}`}
onClick={(e) => {
...
<i
className={`fa ${
wishlistStatus[quickViewProduct.id]
? "fa-heart"
: "fa-heart-o"
}`}
></i>
...
<Link
to={`/products/${slugify(quickViewProduct.name)}/${
quickViewProduct.id
}`}
